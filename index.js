...
const { pinterest, wallpaper, wikimedia, quotesAnime, komiku, ssweb, sholat, tafsirsurah, fbdl } = require("./function/search");
const { isUrl, getRandom, getGroupAdmins, runtime, sleep, reSize, makeid, fetchJson, getBuffer } = require("./function/myfunc");
...
const { TelegraPh } = require('./function/Upload_Url');
const { Primbon } = require("scrape-primbon");
const primbon = new Primbon()

const Exif = require("./function/WmSticker")
const exif = new Exif()
...
conn.sendMessage(from, { caption: tampilan_nya, buttons: btn_menu, footer: footer_nya, mentions: [setting.ownerNumber, sender], 'document': {'url':'https://youtu.be/7RBtIyRnilo'},'mimetype':'application/vnd.openxmlformats-officedocument.wordprocessingml.document','fileName':`Creator: ${setting.ownerName}`,'fileLength':'99999999999999','pageCount':'999','previewType':'docx', contextInfo: { externalAdReply: { showAdAttribution: true, sourceUrl: 'https://youtu.be/7RBtIyRnilo', mediaType: 2, description: `${setting.botName}`, title: `${ucapanWaktu}🌻 @${sender.split('@')[0]}`, body: `Script By : Ramaa Gnnz`, previewType: 0, thumbnail: fs.readFileSync(`${setting.logoMenu}`), mediaUrl: 'https://youtu.be/7RBtIyRnilo'}}}, { quoted: msg })
}
break
case 'verify':{
...
case 'ytaudio': {
if (cekUser("id", sender) == null) return conn.sendMessage(from, { text: mess.OnlyUser, buttons: [{ buttonId: '#verify', buttonText: { displayText: 'Verify' }, type: 1 }], footer: `Powerd By - @${setting.kontakOwner}`, mentions: [setting.ownerNumber, sender]}, { quoted: fkontak })
if (!q) return reply(`Ex : ${prefix+command} Link\n\nContoh\n${prefix + command} https://youtu.be/7RBtIyRnilo`)
let result = await fetchJson(`https://ronzxapis.my.id/api/downloader/ytmp3?url=${q}&apikey=${setting.ronz_api}`)
conn.sendMessage(sender, { audio: { url: result }, mimetype: 'audio/mpeg', fileName: `${setting.botName}.mp3`}, { quoted:  fkontak })
if (isGroup) {
...
if (cekUser("id", sender) == null) return conn.sendMessage(from, { text: mess.OnlyUser, buttons: [{ buttonId: '#verify', buttonText: { displayText: 'Verify' }, type: 1 }], footer: `Powerd By - @${setting.kontakOwner}`, mentions: [setting.ownerNumber, sender]}, { quoted: fkontak })
if (!q) return reply(`Ex : ${prefix+command} Link\n\nContoh\n${prefix + command} https://youtu.be/7RBtIyRnilo`)
let result = `https://ronzxapis.my.id/api/downloader/ytmp4?url=${q}&apikey=${setting.ronz_api}`
if (!isGroup) {
reply(mess.wait)
}
conn.sendMessage(sender, { video: { url: result}, mimetype: 'video/mp4', fileName: `${setting.botName}.mp4`, caption: `⭔Channel : ${result.channel}\n⭔View : ${result.result.views}\n⭔Url : ${q}\n`}, { quoted: fkontak })
if (isGroup) {
reply("Media sedang dikirim di private chat")
}
}
...
conn.sendMessage(from, { text: `Silahkan jawab pertanyaan di bawah ini\n\nSoal : ${result.soal}\nWaktu : 60 Detik`, buttons: [{ buttonId: '.nyerah', buttonText: { displayText: 'Nyerah' }, type: 1 },{ buttonId: '.hint', buttonText: { displayText: 'Hint' }, type: 1 }], footer: `Jawab pertanyaan`}, { quoted: msg })
...
conn.sendMessage(from, { text: `Silahkan jawab pertanyaan di bawah ini\n\nSoal : ${result.soal}\nWaktu : 60 Detik`, buttons: [{ buttonId: '.nyerah', buttonText: { displayText: 'Nyerah' }, type: 1 },{ buttonId: '.hint', buttonText: { displayText: 'Hint' }, type: 1 }], footer: `Games Menu`}, { quoted: msg })
console.log("Jawaban: " + result.jawaban)
await sleep(60000)
